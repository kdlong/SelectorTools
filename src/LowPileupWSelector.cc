...
void LowPileupWSelector::Init(TTree* tree) {
    TParameter<bool>* doSyst =
        (TParameter<bool>*)GetInputList()->FindObject("doSyst");
    ...
    hists1D_ = {
        "CutFlow", "mW",  "mtW",  "mtWUncorr", "yW",       "ptW",
        "phiW",    "ptl", "etal", "pfMet",     "pfMetPhi",
    };
    ...
    metCorrWeightMap_ = {
        {recoilCorrectionEtaShapeUp, 2},   {recoilCorrectionRUShapesUp, 3},
        {recoilCorrectionKeysShapeUp, 4},  {recoilCorrectionStat0Up, 5},
        {recoilCorrectionStat1Up, 6},      {recoilCorrectionStat2Up, 7},
        {recoilCorrectionStat3Up, 8},      {recoilCorrectionStat4Up, 9},
        {recoilCorrectionStat5Up, 10},     {recoilCorrectionStat6Up, 11},
        {recoilCorrectionStat7Up, 12},     {recoilCorrectionStat8Up, 13},
        {recoilCorrectionStat9Up, 14},     {recoilCorrectionEtaShapeDown, 2},
        {recoilCorrectionRUShapesDown, 3}, {recoilCorrectionKeysShapeDown, 4},
        {recoilCorrectionStat0Down, 5},    {recoilCorrectionStat1Down, 6},
        {recoilCorrectionStat2Down, 7},    {recoilCorrectionStat3Down, 8},
        {recoilCorrectionStat4Down, 9},    {recoilCorrectionStat5Down, 10},
        {recoilCorrectionStat6Down, 11},   {recoilCorrectionStat7Down, 12},
        {recoilCorrectionStat8Down, 13},   {recoilCorrectionStat9Down, 14},
    };
...
void LowPileupWSelector::LoadBranchesBacon(Long64_t entry, SystPair variation) {
    ...
    if (isMC_) {
        ...
        if (variation.first == muonEfficiencyMCSubtractUp ||
            ...
            double tempMetPhi =
                metPhiVector.At(metCorrWeightMap_[variation.first]);
            pfMet = tempMet > pfMet ? tempMet : pfMet + (pfMet - tempMet);
            pfMetPhi = tempMet > pfMet ? tempMetPhi
                                       : pfMetPhi + (pfMetPhi - tempMetPhi);
...
void LowPileupWSelector::SetComposite() {
    ...
    wCand = *lep + pfMetVec;
    // mtW =
    // std::sqrt(2*lep->Pt()*pfMet*(1-std::cos(lep->Phi()-pfMetVec.Phi())));
}

void LowPileupWSelector::FillHistograms(Long64_t entry, SystPair variation) {
    if (lep->Pt() < 25 || *mtW < 40) return;
    SafeHistFill(histMap1D_, "mW", channel_, variation.first, wCand.M(),
                 weight);
    SafeHistFill(histMap1D_, "mtW", channel_, variation.first, *mtW, weight);
    SafeHistFill(histMap1D_, "mtWUncorr", channel_, variation.first, *mtWuncorr,
                 weight);
    SafeHistFill(histMap1D_, "ptW", channel_, variation.first, wCand.Pt(),
                 weight);
    SafeHistFill(histMap1D_, "yW", channel_, variation.first, wCand.Rapidity(),
                 weight);
    SafeHistFill(histMap1D_, "phiW", channel_, variation.first, wCand.Phi(),
                 weight);
    SafeHistFill(histMap1D_, "ptl", channel_, variation.first, lep->Pt(),
                 weight);
    SafeHistFill(histMap1D_, "etal", channel_, variation.first, lep->Eta(),
                 weight);
    SafeHistFill(histMap1D_, "pfMet", channel_, variation.first, pfMet, weight);
    SafeHistFill(histMap1D_, "pfMetPhi", channel_, variation.first, pfMetPhi,
                 weight);
    ...
    size_t upperIndex =
        std::distance(binning.begin(),
                      std::upper_bound(binning.begin(), binning.end(), genVPt));
    ...
    try {
        ...
        SafeHistFill(subprocessMap, "mW", channel_, variation.first, wCand.M(),
                     weight);
        SafeHistFill(subprocessMap, "ptW", channel_, variation.first,
                     wCand.Pt(), weight);
        SafeHistFill(subprocessMap, "yW", channel_, variation.first,
                     wCand.Rapidity(), weight);
        SafeHistFill(subprocessMap, "ptl", channel_, variation.first, lep->Pt(),
                     weight);
        SafeHistFill(subprocessMap, "etal", channel_, variation.first,
                     lep->Eta(), weight);
        SafeHistFill(subprocessMap, "pfMet", channel_, variation.first, pfMet,
                     weight);
    } catch (std::exception& e) {
        throw std::range_error("Could not find bin " + binname +
                               " in subprocessMap!");
    }
}

...
void WZBackgroundSelector::SlaveBegin(TTree * /*tree*/) {
    ...
    if (fakeRate_allE_ == nullptr)
        Abort("Must pass electron fake rate to input list!");
    fakeRate_allMu_ =
        (ScaleFactor *)GetInputList()->FindObject("fakeRate_allMu");
    if (fakeRate_allMu_ == nullptr)
        Abort("Must pass muon fake rate to input list!");
}

void WZBackgroundSelector::SetupNewDirectory() {
    WZSelector::SetupNewDirectory();

    // Insure that hist ranges are exactly the same as WZSelector, just change
    // name
    ...
    AddObject<TH1D>(mjjHistPPF_, ("mjj_PPF_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(mjjHistPFP_, ("mjj_PFP_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(mjjHistFPP_, ("mjj_FPP_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(mjjHistFFP_, ("mjj_FFP_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(mjjHistFPF_, ("mjj_FPF_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(mjjHistPFF_, ("mjj_PFF_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(mjjHistFFF_, ("mjj_FFF_" + channelName_).c_str(),
                    "mjj; m_{jj} [GeV]; Events;", 15, 0, 1500);
    AddObject<TH1D>(ZMassHistPPF_, ("ZMass_PPF_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
    AddObject<TH1D>(ZMassHistPFP_, ("ZMass_PFP_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
    AddObject<TH1D>(ZMassHistFPP_, ("ZMass_FPP_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
    AddObject<TH1D>(ZMassHistFFP_, ("ZMass_FFP_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
    AddObject<TH1D>(ZMassHistFPF_, ("ZMass_FPF_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
    AddObject<TH1D>(ZMassHistPFF_, ("ZMass_PFF_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
    AddObject<TH1D>(ZMassHistFFF_, ("ZMass_FFF_" + channelName_).c_str(),
                    "ZMass; m_{Z} [GeV]; Events;", 15, 76, 116);
}
float WZBackgroundSelector::getEventWeight() {
    float evtwgt = 0;
    if (!IsGenMatched3l()) return true;

    if (IsFPPRegion()) {
        if (!isVBS_ || PassesVBSSelection(true)) mjjHistFPP_->Fill(mjj, weight);
    ...
    if (isMC_) evtwgt *= GetPrefiringEfficiencyWeight(jetPt, jetEta);
    return evtwgt;
}

void WZBackgroundSelector::LoadBranchesUWVV(

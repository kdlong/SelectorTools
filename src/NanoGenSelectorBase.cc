...
void NanoGenSelectorBase::Init(TTree* tree) {
    refWeight = 1;
    TParameter<bool>* doTheory =
        (TParameter<bool>*)GetInputList()->FindObject("theoryUnc");
    doTheoryVars_ = doTheory != nullptr && doTheory->GetVal();
    TParameter<bool>* wSignOnly =
        (TParameter<bool>*)GetInputList()->FindObject("wSignOnly");
    weightSignOnly_ = wSignOnly != nullptr && wSignOnly->GetVal();
    TParameter<int>* wSuppress =
        (TParameter<int>*)GetInputList()->FindObject("wSuppress");
    ...
    TParameter<bool>* doPtVSplit =
        (TParameter<bool>*)GetInputList()->FindObject("theoryPtV");
    if (doTheoryVars_ && doPtVSplit != nullptr && doPtVSplit->GetVal()) {
        ...
        std::string selectionName =
            selection == nullptr ? "" : selection->GetTitle();
        SystMap ptvars = {
            {ptV0to3, "ptV0to3"},     {ptV3to5, "ptV3to5"},
            {ptV5to7, "ptV5to7"},     {ptV7to9, "ptV7to9"},
            {ptV9to12, "ptV9to12"},   {ptV12to15, "ptV12to15"},
            {ptV15to20, "ptV15to20"}, {ptV20to27, "ptV20to27"},
            {ptV27to40, "ptV27to40"}, {ptV40toInf, "ptV40toInf"},
        };
    ...
    b.SetTree(tree);
    altScaleWeights_ = (tree->GetListOfBranches()->FindObject(
                            "nLHEScaleWeightAltSet1") != nullptr);
    unknownWeights_ =
        (tree->GetListOfBranches()->FindObject("nLHEUnknownWeight") != nullptr);
    unknownWeightsAlt_ = (tree->GetListOfBranches()->FindObject(
                              "nLHEUnknownWeightAltSet1") != nullptr);
    pdfWeights_ =
        (tree->GetListOfBranches()->FindObject("nLHEPdfWeight") != nullptr);

    SelectorBase::Init(tree);

    edm::FileInPath mc2hessianCSV(
        "PhysicsTools/HepMCCandAlgos/data/NNPDF30_lo_as_0130_hessian_60.csv");
    // Off for now
    doMC2H_ = name_.find("cp5") == std::string::npos && false;
    if (doMC2H_) {
        std::cout
            << "INFO: Will convert MC PDF set to Hessian with MC2Hessian\n";
        pdfweightshelper_.Init(N_LHEPDF_WEIGHTS_, N_MC2HESSIAN_WEIGHTS_,
                               mc2hessianCSV);
    }
    centralWeightIndex_ = -1;
    // NNLOPSLike is just a config name for one MiNNLO sample
    if (name_.find("nnlops") != std::string::npos &&
        name_.find("nnlopslike") == std::string::npos) {
        if (name_.find("nloOnly") == std::string::npos) {
            centralWeightIndex_ = 9;
            std::cout
                << "INFO: NNLOPS sample will be weighted by NNLO weight\n";
        } else
    ...
    // doBorn_ = std::find_if(systematics_.begin(), systematics_.end(), [](auto&
    // s) { return s.first == BornParticles; }) != systematics_.end();
    // doBareLeptons_ = std::find_if(systematics_.begin(), systematics_.end(),
    // [](auto& s) { return s.first == BareLeptons; }) != systematics_.end();
    doBorn_ = true;
    doBareLeptons_ = true;
...
void NanoGenSelectorBase::SetBranchesNanoAOD() {
    ...
    if (unknownWeights_) {
        ...
        if (unknownWeightsAlt_) {
            b.SetSpecificBranch("nLHEUnknownWeightAltSet1",
                                nLHEUnknownWeightAltSet1);
            b.SetSpecificBranch("LHEUnknownWeightAltSet1",
                                LHEUnknownWeightAltSet1);
        }
    }
}

void NanoGenSelectorBase::LoadBranchesNanoAOD(Long64_t entry,
                                              SystPair variation) {
    weight = 1;
    if (altScaleWeights_) {
    ...
    if (!altScaleWeights_) nLHEScaleWeightAltSet1 = 0;
    if (!pdfWeights_) nLHEPdfWeight = 0;
    if (!unknownWeights_) nLHEUnknownWeight = 0;
    if (!unknownWeightsAlt_) nLHEUnknownWeightAltSet1 = 0;
    fReader.SetLocalEntry(entry);

    channel_ = channelMap_[channelName_];

    // Sort descending
    auto compareMaxByPt = [](const reco::GenParticle& a,
                             const reco::GenParticle& b) {
        return a.pt() > b.pt();
    };
    std::vector<unsigned int> idsToKeep = {11, 12, 13, 14};
    if (doPhotons_) idsToKeep.push_back(22);

    // This is a bit nasty because it assumes that Central is the first
    // variation, which is NOT guaranteed!
    if (variation.first == Central) {
        ...
        for (size_t i = 0; i < *nGenDressedLepton; i++) {
            ...
            dressedLeptons.emplace_back(makeGenParticle(
                GenDressedLepton_pdgId.At(i), 1, GenDressedLepton_pt.At(i),
                GenDressedLepton_eta.At(i), GenDressedLepton_phi.At(i),
                GenDressedLepton_mass.At(i)));
        ...
        if (doNeutrinos_ || doBorn_ || doBareLeptons_) {
            for (size_t i = 0; i < *nGenPart; i++) {
                ...
                if (!(isHardProcess || (GenPart_status.At(i) == 1 && isPrompt)))
                    continue;
                if (std::find(idsToKeep.begin(), idsToKeep.end(),
                              std::abs(GenPart_pdgId.At(i))) == idsToKeep.end())
                    continue;

                auto part = makeGenParticle(
                    GenPart_pdgId.At(i), GenPart_status.At(i), GenPart_pt.At(i),
        ...
        if (bareLeptons.size() > 0 && doPhotons_) {
            ...
            photons.erase(std::remove_if(photons.begin(), photons.end(),
                                         [lep](const reco::GenParticle& p) {
                                             return reco::deltaR(p, lep) > 0.1;
                                         }),
                          photons.end());
        }
        ...
        for (size_t i = 0; i < *nLHEPart; i++) {
            if (std::find(idsToKeep.begin(), idsToKeep.end(),
                          std::abs(LHEPart_pdgId.At(i))) == idsToKeep.end())
                continue;

            auto part = makeGenParticle(LHEPart_pdgId.At(i), 1,
                                        LHEPart_pt.At(i), LHEPart_eta.At(i),
                                        LHEPart_phi.At(i), LHEPart_mass.At(i));
            ...
            else if ((std::abs(part.pdgId()) < 6 ||
                      std::abs(part.pdgId()) == 21) &&
                     part.pt() > 30) {
    ...
    if (variation.first != LHEParticles) {
        ...
        for (size_t i = 0; i < *nGenJet; i++) {
            auto jet = makeGenParticle(0, 1, GenJet_pt.At(i), GenJet_eta.At(i),
                                       GenJet_phi.At(i), GenJet_mass.At(i));
            if (jet.pt() > 30 && !helpers::overlapsCollection(
                                     jet.polarP4(), leptons, 0.4, nLeptons_)) {
                ht += jet.pt();
    ...
    weight = *genWeight;
    if (weightSignOnly_) weight /= std::abs(*genWeight);
    ...
    if (refWeight == 1) refWeight = weight;

    if (centralWeightIndex_ != -1) {
        weight *= LHEScaleWeight.At(centralWeightIndex_);
    }
    if (doMC2H_) buildHessian2MCSet();

    SetComposite();
}

reco::GenParticle NanoGenSelectorBase::makeGenParticle(int pdgid, int status,
                                                       float pt, float eta,
                                                       float phi, float m) {
    LorentzVector vec;
    vec.SetPt(pt);
    ...
    if (std::abs(pdgid) == 12 || std::abs(pdgid) == 14 ||
        std::abs(pdgid) == 16 || std::abs(pdgid) == 22 || std::abs(pdgid) == 23)
        charge = 0;

    auto lep = reco::GenParticle(charge, vec, reco::Particle::Point(), pdgid,
                                 status, true);
    return lep;
}
...
void NanoGenSelectorBase::buildHessian2MCSet() {
    ...
    //    }
    pdfweightshelper_.DoMC2Hessian(1., const_cast<const double*>(pdfWeights),
                                   LHEHessianPdfWeight);
}
...
void NanoGenSelectorBase::SetupNewDirectory() {
    ...
    AddObject<TH1D>(hesPdfWeights_, "Hesweights", "Hessian pdf weights", 200, 0,
                    2);

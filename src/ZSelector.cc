...
void ZSelector::Init(TTree *tree) {
    ...
    hists1D_ = {
        "CutFlow", "ZMass", "ZEta",  "yZ",    "ZPt",   "ptl1",  "etal1",
        "ptl2",    "etal2", "ptj1",  "ptj2",  "ptj3",  "etaj1", "etaj2",
        "etaj3",   "phij1", "phij2", "phij3", "nJets", "MET",
    };
...
void ZSelector::SetBranchesUWVV() {
    ...
    fChain->SetBranchAddress("type1_pfMETPhi", &type1_pfMETPhi,
                             &b_type1_pfMETPhi);
    fChain->SetBranchAddress("nZZTightElec", &nCBVIDTightElec,
                             &b_nCBVIDTightElec);
    // fChain->SetBranchAddress("nCBVIDHLTSafeElec", &nCBVIDHLTSafeElec,
    // &b_nCBVIDHLTSafeElec);
    fChain->SetBranchAddress("nZZTightMu", &nTightIdMuon, &b_nTightIdMuon);
    // fChain->SetBranchAddress("nWZMediumMuon", &nWZMediumMuon,
    // &b_nWZMediumMuon);
}
...
void ZSelector::SetBranchesNanoAOD() {
    ...
    // b.SetBranch("HLT_Ele17_Ele12_CaloIdL_TrackIdL_IsoVL_DZ",
    // Dielectron_Trigger);
...
void ZSelector::LoadBranchesNanoAOD(
    Long64_t entry, std::pair<Systematic, std::string> variation) {
    weight = 1;
    b.SetEntry(entry);

    // You could resize the array per event to match the number number of
    // muons/electrons read but it doesn't seem worth it to me. Just set to a
    // high enough value
    ...
    if (nJet > N_KEEP_JET_) {
        std::string message =
            "Found more jets than max read number.\n    Found ";
        message += std::to_string(nJet);
        message += " particles\n  --> Max read number was ";
        message += std::to_string(N_KEEP_JET_);
        message +=
            "\nExiting because this can cause problems. Increase N_KEEP_JET_ "
            "to avoid this error.\n";
        throw std::domain_error(message);
    }
    ...
    nCBVIDTightElec =
        std::count(Electron_cutBased, Electron_cutBased + nElectron, 4);
    nCBVIDVetoElec =
        std::count(Electron_cutBased, Electron_cutBased + nElectron, 1);
    nTightIdMuon = 0;
    nMediumIdMuon = 0;
    ...
    if (nTightIdMuon >= 2) {
        ...
        if (!(Muon_mediumId[0] && Muon_pfRelIso04_all[0] < 0.15 &&
            ...
            for (size_t i = 0; i < nMuon; i++) {
                if (Muon_mediumId[i]) goodIndices.push_back(i);
            }
        ...
        if (Muon_charge[goodIndices[0]] != Muon_charge[goodIndices[1]]) {
            ...
            l1IsTight = (Muon_mediumId[goodIndices[0]] &&
                         (Muon_pfRelIso04_all[goodIndices[0]] < 0.15));
            l2IsTight = (Muon_mediumId[goodIndices[1]] &&
                         (Muon_pfRelIso04_all[goodIndices[1]] < 0.15));
        }
        ...
        if (!(Electron_cutBased[0] == 4 && Electron_cutBased[1] == 4)) {
            for (size_t i = 0; i < nElectron; i++) {
                if (Electron_cutBased[i] == 4) goodIndices.push_back(i);
            }
            if (goodIndices.size() < 2) return;
        } else
            goodIndices = {0, 1};
        if (Electron_charge[goodIndices[0]] !=
            Electron_charge[goodIndices[1]]) {
    ...
    for (size_t i = 0; i < nJet; i++) {
        ...
        jet.SetM(Jet_mass[i]);
        if (jet.pt() > 30 &&
            !helpers::overlapsCollection(jet, leptons, 0.4, leptons.size()))
            jets.push_back(jet);
    }
    SetComposite();
...
void ZSelector::LoadBranchesUWVV(Long64_t entry,
...
bool ZSelector::zlep1IsTight() { return l1IsTight; }

bool ZSelector::zlep2IsTight() { return l2IsTight; }

bool ZSelector::tightZLeptons() { return zlep1IsTight() && zlep2IsTight(); }

void ZSelector::FillHistograms(Long64_t entry,
                               std::pair<Systematic, std::string> variation) {
    int step = 0;
    SafeHistFill(histMap1D_, "CutFlow", channel_, variation.first, step++,
                 weight);

    if (channel_ != mm && channel_ != ee) return;
    SafeHistFill(histMap1D_, "CutFlow", channel_, variation.first, step++,
                 weight);

    if (!passesTrigger) return;
    SafeHistFill(histMap1D_, "CutFlow", channel_, variation.first, step++,
                 weight);
    ...
    if (l1Pt < 25 || l2Pt < 25) return;
    SafeHistFill(histMap1D_, "CutFlow", channel_, variation.first, step++,
                 weight);

    if (ZMass > 106.1876 || ZMass < 76.1876) return;
    SafeHistFill(histMap1D_, "CutFlow", channel_, variation.first, step++,
                 weight);

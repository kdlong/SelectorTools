...
void SelectorBase::SlaveBegin(TTree* /*tree*/) {
    if (GetInputList() != nullptr) {
        TParameter<bool>* applyScaleFactors =
            (TParameter<bool>*)GetInputList()->FindObject("applyScaleFacs");
...
void SelectorBase::Init(TTree* tree) {
    ...
    if (GetInputList() != nullptr) {
        ...
        if (name_.empty()) {
            std::cerr << "INFO: Using default name \"Unknown\" for file"
                      << std::endl;
            name_ = "Unknown";
        }
        if (year != nullptr) {
    ...
    if (selectionMap_.find(selectionName_) != selectionMap_.end()) {
        ...
        throw std::invalid_argument(selectionName_ +
    ...
    if (channelMap_.find(channelName_) != channelMap_.end())
        ...
        for (const auto& chan : channelMap_) message += chan.first + ", ";
...
void SelectorBase::setSubprocesses(std::string process) {
    currentHistDir_ =
        dynamic_cast<TList*>(fOutput->FindObject(process.c_str()));
    if (currentHistDir_ == nullptr) {
        currentHistDir_ = new TList();
        currentHistDir_->SetName(process.c_str());
        std::cout << "Info: Setting output dir name to " << process
                  << std::endl;
        fOutput->Add(currentHistDir_);
    }
    // throw std::invalid_argument(process + " is not a valid subprocess for
    // selector!");
}

void SelectorBase::makeOutputDirs() {
    std::vector<std::string> allProcesses = {name_};
    allProcesses.insert(allProcesses.begin(), subprocesses_.begin(),
                        subprocesses_.end());

    for (auto& name : allProcesses) {
        currentHistDir_ =
            dynamic_cast<TList*>(fOutput->FindObject(name.c_str()));

        if (currentHistDir_ == nullptr) {
            currentHistDir_ = new TList();
            currentHistDir_->SetName(name.c_str());
            fOutput->Add(currentHistDir_);
            if (name == name_) SetupNewDirectory();
        }
    }
    ...
    // if ( existingObjectPtrsSize > 0 && allObjects_.size() !=
    // existingObjectPtrsSize ) {
    //    throw std::invalid_argument(Form("SelectorBase: Size of allObjects has
    //    changed!: %lu to %lu", existingObjectPtrsSize, allObjects_.size()));
    //}
    // UpdateDirectory();
}
...
float SelectorBase::GetPrefiringEfficiencyWeight(std::vector<float>* jetPt,
                                                 std::vector<float>* jetEta) {
    float prefire_weight = 1;
    for (size_t i = 0; i < jetPt->size(); i++) {
        float jPt = jetPt->at(i);
        float jEta = std::abs(jetEta->at(i));
        prefire_weight *= (1 - prefireEff_->GetEfficiency(
                                   prefireEff_->FindFixBin(jEta, jPt)));
    }
    ...
    for (TNamed** objPtrPtr : allObjects_) {
        if (*objPtrPtr == nullptr)
            std::invalid_argument(
                "SelectorBase: Call to UpdateObject but existing pointer is "
                "null");
        *objPtrPtr =
            (TNamed*)currentHistDir_->FindObject((*objPtrPtr)->GetName());
        if (*objPtrPtr == nullptr)
            std::invalid_argument(
                "SelectorBase: Call to UpdateObject but current directory has "
                "no instance");
    }
}

template <typename T>
void SelectorBase::InitializeHistMap(
    std::vector<std::string>& labels,
    std::unordered_map<HistLabel, T*>& histMap) {
...
void SelectorBase::InitializeHistogramsFromConfig() {
    ...
    if (histInfo == nullptr)
        throw std::domain_error(
            "Can't initialize histograms without passing histogram information "
            "to TSelector");
    ...
    for (auto&& entry : *histInfo) {
        ...
        std::vector<std::string> histData =
            ReadHistDataFromConfig(currentHistInfo->GetTitle());
        ...
        for (const auto& chan : channels) {
            ...
            if (histMap1D_.find(centralLabel) != histMap1D_.end() ||
                histMap2D_.find(centralLabel) != histMap2D_.end()) {
                InitializeHistogramFromConfig(name, chan, histData);
            }
            // May add a syst hist to plotobjects for plotting purposes,
            // surpress errors in this case
            else if (std::find(tempSystHistNames.begin(),
                               tempSystHistNames.end(),
                               name) == tempSystHistNames.end())
                std::cerr << "Skipping invalid histogram '" << name << "'"
                          << std::endl;
        }
    }
    ...
    for (auto& subprocess : subprocesses_) {
        ...
        for (auto& hist : histMap1D_) {
            ...
            if (hist.second == nullptr) continue;
            AddObject<TH1D>(subprocessMap[hist.first],
                            const_cast<TH1D&>(*hist.second));
...
void SelectorBase::InitializeHistogramFromConfig(
    std::string name, ChannelPair channel, std::vector<std::string>& histData) {
    if (histData.size() != 4 && histData.size() != 7) {
        std::cerr
            << "Malformed data string for histogram '" << name
            << ".' Must have form: 'Title; (optional info) $ nbins, xmin, xmax'"
            << "\n   OR form: 'Title; (optional info) $ nbins, xmin, xmax "
               "nbinsy ymin ymax'"
            << std::endl;
        exit(1);
    }
    ...
    for (auto& variation : variations_) {
        std::string histName =
            getHistName(name, variation.second, channel.second);
        ...
        if (variation.first == Central) {
            if (is1D)
                ...
                AddObject<TH2D>(histMap2D_[histlabel], histName.c_str(),
                                histData.at(0).c_str(), nbins, xmin, xmax,
                                nbinsy, ymin, ymax);
        } else if ((is1D && std::find(systHists_.begin(), systHists_.end(),
                                      name) == systHists_.end()) ||
                   (!is1D && std::find(systHists2D_.begin(), systHists2D_.end(),
                                       name) == systHists2D_.end()))
            continue;
        ...
        if (std::find(theoryVarSysts_.begin(), theoryVarSysts_.end(),
            ...
            else if (std::find(weighthists2D_.begin(), weighthists2D_.end(),
                               name) != weighthists2D_.end()) {
                weighthistMap2D_[histlabel] = {};
                AddObject<TH3D>(weighthistMap2D_[histlabel],
                                weighthistName.c_str(), histData[0].c_str(),
                                nbins, xmin, xmax, nbinsy, ymin, ymax, nweights,
                                0, nweights);
            }
        }
    }
...
std::vector<std::string> SelectorBase::ReadHistDataFromConfig(
    std::string histDataString) {
    std::vector<std::string> histData;
    boost::split(histData, histDataString, boost::is_any_of("$"));
    std::vector<std::string> binInfo;
    if (histData.size() != 2) return {};
...
std::string SelectorBase::concatenateNames(const std::string& baseName,
                                           std::string& toAppend) {
...
std::string SelectorBase::concatenateNames(const char* baseName,
                                           std::string& toAppend) {
    if (toAppend.empty()) return baseName;
...
std::string SelectorBase::getHistName(std::string histName,

...
void ScaleFactor::Set1DHist(TH1D *central, TH1D *shiftUp, TH1D *shiftDown,
                            OverflowBehavior b) {
...
void ScaleFactor::Set2DHist(TH2D *central, TH2D *shiftUp, TH2D *shiftDown,
                            OverflowBehavior b) {
...
double ScaleFactor::Evaluate1D(double x, Variation var) const {
    if (var == CentralValue && histCentral1D_ != nullptr) {
        ...
        auto bin = histCentral1D_->FindBin(x);
        return histCentral1D_->GetBinContent(bin) +
               histCentral1D_->GetBinError(bin);
...
double ScaleFactor::Evaluate2D(double x, double y, Variation var) const {
    if (var == CentralValue && histCentral2D_ != nullptr) {
        ...
        auto bin = histCentral2D_->FindBin(x, y);
        return histCentral2D_->GetBinContent(bin) +
               histCentral2D_->GetBinError(bin);
...
void ScaleFactor::RegisterGlobalFunction(int dimension) {
    ...
    if (prev != nullptr && prev != (TObject *)this) {
        Error("ScaleFactor",
              "RegisterGlobalFunction(): There is an object with this name "
              "already in global specials");
        return;
    ...
    if (interpreter == nullptr) {
        Error(
            "ScaleFactor",
            "RegisterGlobalFunction() only works in interactive ROOT context");
    }

    if (dimension == 1) {
        TString fDecl(
            Form("double %s(double x) { auto ptr = (ScaleFactor*) "
                 "gROOT->GetListOfSpecials()->FindObject(\"%s\"); return "
                 "ptr->Evaluate1D(x); }",
                 GetName(), GetName()));
        interpreter->Declare(fDecl);
    } else if (dimension == 2) {
        TString fDecl(
            Form("double %s(double x, double y) { auto ptr = (ScaleFactor*) "
                 "gROOT->GetListOfSpecials()->FindObject(\"%s\"); return "
                 "ptr->Evaluate2D(x, y); }",
                 GetName(), GetName()));
        interpreter->Declare(fDecl);
        ...
        Error("ScaleFactor",
              "Called RegisterGlobalFunction() with unsupported dimension (%d)",
              dimension);
    }
}

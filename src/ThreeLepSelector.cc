...
void ThreeLepSelector::SetScaleFactors() {
    ...
    if (pileupSF_ == nullptr) std::cout << "missing Pileup SF" << std::endl;
    ...
    if (eIdSF_ == nullptr) std::cout << "missing Electron ID SF" << std::endl;

    mIdSF_ = (ScaleFactor*)GetInputList()->FindObject("muonMediumIdSF");
    if (mIdSF_ == nullptr) std::cout << "missing Muon Id SF" << std::endl;

    TFile* f_btag_eff = new TFile(
        "ScaleFactors/"
        "btageff__ttbar_powheg_pythia8_25ns_Moriond17_deepCSV.root");
...
void ThreeLepSelector::Init(TTree* tree) {
    ...
    allChannels_ = {
        {mm, "mm"}, {ee, "ee"}, {em, "em"}, {all, "all"}, {lll, "lll"}};

    hists1D_ = {
        "CutFlow",    "ZMass",  "ptl1",      "etal1",    "ptl2",
        "etal2",      "SR",     "bjetpt",    "jetpt",    "nbjet",
        "njet",       "nleps",  "CRW_nbjet", "CRW_njet", "CRZ_nbjet",
        "CRZ_njet",   "CRZ_HT", "CRZ_Met",   "Met",      "HT",
        "weight",     "CRW_HT", "CRW_Met",   "CRZ_ptl3", "sphericity",
        "centrality",
    };
...
void ThreeLepSelector::SetBranchesNanoAOD() {
    ...
    b.SetBranch("HLT_DoubleEle8_CaloIdM_TrackIdM_Mass8_PFHT300",
    ...
    b.SetBranch("Electron_dr03HcalDepth1TowerSumEt",
    ...
    b.SetBranch("Flag_globalSuperTightHalo2016Filter",
    ...
    mvaValues[CBID_LOOSE] = {{-0.46, -0.48, 0, -0.85},
                             {-0.03, -0.67, 0, -0.91},
                             {0.06, -0.49, 0, -0.83}};
    mvaValues[CBID_TIGHT] = {
        {10, 0.77, 0, 0.52}, {10, 0.56, 0, 0.11}, {10, 0.48, 0, -0.01}};
...
void ThreeLepSelector::LoadBranchesNanoAOD(
    ...
    if (nElectron > N_KEEP_MU_E_ || nMuon > N_KEEP_MU_E_ ||
        ...
        message +=
            "\nExiting because this can cause problems. Increase N_KEEP_GEN_ "
            "to avoid this error.\n";
...
void ThreeLepSelector::setupMuons() {
    for (size_t i = 0; i < nMuon; ++i) {
        if (isGoodMuon(i)) {
            goodLeptons.push_back(
                GoodPart(get4Vector(PID_MUON, i), PID_MUON * Muon_charge[i]));
            goodLeptons.back().index = i;
            looseLeptons.push_back(goodLeptons.back());

            if (!passFullIso(goodLeptons.back().v, 0.76,
                             7.2)) {  // Extra Iso requirement
                goodLeptons.pop_back();
            }
        } else if (isLooseMuon(i)) {
            looseLeptons.push_back(
                GoodPart(get4Vector(PID_MUON, i), PID_MUON * Muon_charge[i]));
...
void ThreeLepSelector::setupElectrons() {
    for (size_t i = 0; i < nElectron; ++i) {
        if (isGoodElectron(i)) {
            goodLeptons.push_back(GoodPart(get4Vector(PID_ELECTRON, i),
                                           PID_ELECTRON * Electron_charge[i]));
            goodLeptons.back().index = i;
            looseLeptons.push_back(goodLeptons.back());

            if (!passFullIso(goodLeptons.back().v, 0.8,
                             7.2)) {  // Extra Iso requirement
                goodLeptons.pop_back();
            }
        } else if (isLooseElectron(i)) {
            looseLeptons.push_back(GoodPart(get4Vector(PID_ELECTRON, i),
                                            PID_ELECTRON * Electron_charge[i]));
            looseLeptons.back().index = i;
        }
    }
...
void ThreeLepSelector::setupJets() {
    ...
    for (auto lep : looseLeptons) {
        if (passFakeableCuts(lep))
            closeJet.push_back(getCloseJetIndex(lep.v, 0.4));
    }

    for (size_t i = 0; i < nJet; ++i) {
        // if(goodLeptons.size() < 2) break;  // only try to find jets if have
        // leptons
        if (std::find(closeJet.begin(), closeJet.end(), i) != closeJet.end())
            continue;
        /// jet
        ...
        if (isGoodJet(i)) {
            passedGoodJet = true;
            goodJets.push_back(
                GoodPart(get4Vector(PID_JET, i), Jet_hadronFlavour[i]));
        ...
        if (isGoodBJet(i)) {
            if (!passedGoodJet) {
                goodJets.push_back(
                    GoodPart(get4Vector(PID_BJET, i), Jet_hadronFlavour[i]));
                goodJets.back().index = i;
            }
...
void ThreeLepSelector::setupChannel() {
    ...
    } else if (goodLeptons[0].Id() == PID_MUON &&
               goodLeptons[1].Id() == PID_MUON) {
        channelName_ = "mm";
    } else if (goodLeptons[0].Id() == PID_ELECTRON &&
               goodLeptons[1].Id() == PID_ELECTRON) {
        channelName_ = "ee";
    } else {
        channelName_ = "em";
        if (goodLeptons[0].Pt() < goodLeptons[1].Pt()) {
...
LorentzVector ThreeLepSelector::get4Vector(PID pid, int idx) {
    if (pid == PID_MUON)
        return LorentzVector(Muon_pt[idx], Muon_eta[idx], Muon_phi[idx],
                             Muon_mass[idx]);
    else if (pid == PID_ELECTRON)
        return LorentzVector(Electron_pt[idx] / CHGPT(idx), Electron_eta[idx],
                             Electron_phi[idx], Electron_mass[idx]);
    else
        return LorentzVector(Jet_pt[idx], Jet_eta[idx], Jet_phi[idx],
                             Jet_mass[idx]);
}

bool ThreeLepSelector::doesPassZVeto(GoodPart& lep,
                                     std::vector<GoodPart>& looseList) {
...
void ThreeLepSelector::ApplyScaleFactors() {
    ...
    weight *= triggerScaleFactor(
        goodLeptons[0].Id(), goodLeptons[1].Id(), goodLeptons[0].Pt(),
        goodLeptons[1].Pt(), goodLeptons[0].Eta(), goodLeptons[1].Eta(), HT);
    ...
    for (auto jet : goodJets) {
        ...
        else if (jet.Id() == PID_CJET)
            flav = BTagEntry::FLAV_C;
        else
            flav = BTagEntry::FLAV_UDSG;
        double bSF =
            btag_reader.eval_auto_bounds("central", flav, jet.Eta(), jet.Pt());
...
bool ThreeLepSelector::passFakeableCuts(GoodPart& lep) {
    ...
    if (lep.Id() == PID_MUON) {
        return (Muon_mediumId[index] && Muon_sip3d[index] < 4 &&
                Muon_tightCharge[index] == 2 && passFullIso(lep.v, 0.72, 7.2)

        );
    } else {
        return (Electron_sip3d[index] < 4 && Electron_tightCharge[index] == 2 &&
                Electron_lostHits[index] == 0 && passFullIso(lep.v, 0.8, 7.2));
    }
}
...
bool ThreeLepSelector::isGoodMuon(size_t index) {
    bool yearCuts = true;
    if (year_ == yr2016)
        yearCuts = (Muon_miniPFRelIso_all[index] < 0.16);
    else
        yearCuts = (Muon_miniPFRelIso_all[index] < 0.11);

    return ((Muon_pt[index] > 20) && (Muon_tightCharge[index] == 2) &&
            (abs(Muon_eta[index]) < 2.4) && (Muon_mediumId[index]) &&
            (yearCuts) && (abs(Muon_dz[index]) < 0.1) &&
            (abs(Muon_dxy[index]) < 0.05) && (Muon_sip3d[index] < 4));
}

bool ThreeLepSelector::passMVACut(std::vector<std::vector<double>> mvaCuts,
                                  int index) {
    int caseIndex = 0;
    //// PT Splitting
    if (Electron_pt[index] / CHGPT(index) < 5)
        return false;
    else if (Electron_pt[index] / CHGPT(index) < 10)
        caseIndex += 0;
    else if (Electron_pt[index] / CHGPT(index) < 15)
        caseIndex += 1;
    else if (Electron_pt[index] / CHGPT(index) < 25)
        caseIndex += 2;
    else
        caseIndex += 3;
    //// ETA Splitting
    ...
    if (caseIndex % 4 != 2)
        return Electron_MVA[index] > mvaCuts[caseIndex / 4][caseIndex % 4];
    else
        return Electron_MVA[index] >
               mvaInterpolate(Electron_pt[index] / CHGPT(index),
                              mvaCuts[caseIndex / 4]);
}
...
bool ThreeLepSelector::isGoodElectron(size_t index) {
    ...
    if (selection_ == FourTopMVAEl || selection_ != FourTopCutBasedEl) {
        ...
        //	if(year_ == yr2016)       passId = passId &&
        //(Electron_miniPFRelIso_all[index] < 0.12); 	else if(year_ == yr2017)
        //passId = passId && (Electron_miniPFRelIso_all[index] < 0.07);
    }

    return (
        (Electron_pt[index] / CHGPT(index) > 20) && (passId) &&
        (Electron_convVeto[index]) && (Electron_lostHits[index] == 0) &&
        (Electron_tightCharge[index] == 2) && (abs(Electron_dz[index]) < 0.1) &&
        (abs(Electron_dxy[index]) < 0.05) && (Electron_sip3d[index] < 4) &&
        passTriggerEmu(index) &&
        Electron_dr03EcalRecHitSumEt[index] / Electron_pt[index] *
                CHGPT(index) <
            0.45 &&
        Electron_dr03HcalDepth1TowerSumEt[index] / Electron_pt[index] *
                CHGPT(index) <
            0.25 &&
...
bool ThreeLepSelector::isLooseMuon(size_t index) {
    return ((Muon_isGlobal[index] || Muon_isTracker[index]) &&
            (Muon_isPFcand[index]) && (Muon_miniPFRelIso_all[index] < 0.4) &&
            (abs(Muon_dz[index]) < 0.1) && (abs(Muon_dxy[index]) < 0.05) &&
            (Muon_pt[index] > 5));
}
...
bool ThreeLepSelector::isLooseElectron(size_t index) {
    ...
    return ((passId)  //
            ...
            (Electron_convVeto[index]) && (Electron_lostHits[index] <= 1) &&
            (Electron_miniPFRelIso_all[index] < 0.4) && passTriggerEmu(index) &&
...
bool ThreeLepSelector::isGoodJet(size_t index) {
    ...
    if (year_ == yr2016)
        yearCut = IntBits(Jet_jetId[index]).test(0);
    else
        yearCut = IntBits(Jet_jetId[index]).test(1);
    return ((Jet_pt[index] > ptCut) && (abs(Jet_eta[index]) < 2.4) &&
            (yearCut));
}
...
#ifdef CLOSEJET_REWEIGHT
    closeJet =
        (Jet_L1[closeIdx] * (1 - Jet_rawFactor[closeIdx]) * closeJet - lep) *
            Jet_L2L3[closeIdx] +
        lep;
#endif  // CLOSEJET_REWEIGHT
...
bool ThreeLepSelector::passTriggerEmu(size_t index) {
    ...
    if (abs(Electron_eta[index]) < 1.479) {
        etaDepend = Electron_sieie[index] < 0.011 && (DEtaInCut >= 1) &&
                    (DPhiInCut >= 4);
    } else {
        etaDepend = Electron_sieie[index] < 0.031 && (DEtaInCut >= 1) &&
                    (DPhiInCut >= 3);
    }
    return (abs(Electron_eInvMinusPInv[index]) < 0.01 && etaDepend &&
...
bool ThreeLepSelector::MetFilter() {
    return Flag_goodVertices && Flag_globalSuperTightHalo2016Filter &&
           Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter &&
           Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter &&
           Flag_ecalBadCalibFilter;
}

void ThreeLepSelector::FillHistograms(
    Long64_t entry, std::pair<Systematic, std::string> variation) {
    int step = 0;
    Fill1D("CutFlow", 0);
    ...
    if ((goodLeptons.size() == 2 &&
         goodLeptons[0].Charge() * goodLeptons[1].Charge() < 0) ||
        (goodLeptons.size() == 3 &&
         goodLeptons[0].Charge() * goodLeptons[2].Charge() > 0))
        return;
    Fill1D("CutFlow", ++step);
...
int ThreeLepSelector::getSRBin() const {
    if (goodLeptons.size() == 2) {
        ...
        if (!passZVeto)
            return 9;  /// ZCR
        else if (nBJets == 2 && nJets >= 5)
            return 7;
        else if (nBJets >= 3 && nJets >= 4)
            return 8;
    }
...
float ThreeLepSelector::getBtagEffFromFile(double pt, double eta,
                                           int mcFlavour) {
    float pt_cutoff = std::max(20., std::min(399., pt));
    if (abs(mcFlavour) == 5) {
        // use pt bins up to 600 GeV for b
        pt_cutoff = std::max(20., std::min(599., pt));
        return h_btag_eff_b->GetBinContent(
            h_btag_eff_b->FindBin(pt_cutoff, fabs(eta)));
    } else if (abs(mcFlavour) == 4) {
        return h_btag_eff_c->GetBinContent(
            h_btag_eff_c->FindBin(pt_cutoff, fabs(eta)));
    } else {
        return h_btag_eff_udsg->GetBinContent(
            h_btag_eff_udsg->FindBin(pt_cutoff, fabs(eta)));
    }
}
...
double ThreeLepSelector::getWDecayScaleFactor() {
    ...
    return pow((pdgleptW / genleptW), nleptonicW) *
           pow(((1 - pdgleptW) / (1 - genleptW)), nhadronicW);
}

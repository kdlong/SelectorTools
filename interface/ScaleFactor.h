...
class ScaleFactor : public TNamed {
   public:
    ...
    enum OverflowBehavior { AsInHist, NearestEntry, Unity };

    ScaleFactor() {}
    ScaleFactor(const char *name, const char *title) : TNamed(name, title) {}
    ScaleFactor(const TString &name, const TString &title)
        : TNamed(name, title) {}
    virtual ~ScaleFactor();

    void Set1DHist(TH1D *central, TH1D *shiftUp = 0, TH1D *shiftDown = 0,
                   OverflowBehavior b = Unity);
    TH1D *Get1DHist(Variation var = CentralValue);
    void Set2DHist(TH2D *central, TH2D *shiftUp = 0, TH2D *shiftDown = 0,
                   OverflowBehavior b = Unity);

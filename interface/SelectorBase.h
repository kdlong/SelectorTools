...
enum Systematic {
    ...
    pileupUp,
    pileupDown,
    muonEfficiencyMCSubtractUp,
    muonEfficiencyMCSubtractDown,
    modelingFsrUp,
    modelingFsrDown,
    muonEfficiencyBackgroundUp,
    muonEfficiencyBackgroundDown,
    muonEfficiencyTagPtUp,
    muonEfficiencyTagPtDown,
    muonEfficiencyStatUp,
    muonEfficiencyStatDown,
    recoilCorrectionEtaShapeUp,
    recoilCorrectionEtaShapeDown,
    recoilCorrectionRUShapesUp,
    recoilCorrectionRUShapesDown,
    recoilCorrectionKeysShapeUp,
    recoilCorrectionKeysShapeDown,
    recoilCorrectionStat0Up,
    recoilCorrectionStat0Down,
    recoilCorrectionStat1Up,
    recoilCorrectionStat1Down,
    recoilCorrectionStat2Up,
    recoilCorrectionStat2Down,
    recoilCorrectionStat3Up,
    recoilCorrectionStat3Down,
    recoilCorrectionStat4Up,
    recoilCorrectionStat4Down,
    recoilCorrectionStat5Up,
    recoilCorrectionStat5Down,
    recoilCorrectionStat6Up,
    recoilCorrectionStat6Down,
    recoilCorrectionStat7Up,
    recoilCorrectionStat7Down,
    recoilCorrectionStat8Up,
    recoilCorrectionStat8Down,
    recoilCorrectionStat9Up,
    recoilCorrectionStat9Down,
    BareLeptons,
    BornParticles,
    LHEParticles,
    mWShift100MeVUp,
...
struct HistLabel {
    ...
    bool operator==(const HistLabel& h) const {
        return (name == h.name && channel == h.channel &&
...
class SelectorBase : public TSelector {
   public:
    ...
    enum Year { yrdefault, yr2016, yr2017, yr2018 };
    ...
    std::map<std::string, Channel> channelMap_ = {
        ...
        {"mp", mp},     {"ep", ep},     {"mn", mn},
        {"ee", ee},     {"em", em},     {"mm", mm},
        {"eee", eee},   {"eem", eem},   {"emm", emm},
        {"mmm", mmm},   {"eeee", eeee}, {"eemm", eemm},
        {"mmee", mmee}, {"mmmm", mmmm}, {"Inclusive", Inclusive},
        {"lll", lll},   {"all", all},
    };
    ...
    void AddObject(T*& ptr, Args... args) {
        static_assert(std::is_base_of<TNamed, T>::value,
                      "Objects must inheirit from ROOT TNamed to be streamable "
                      "from PROOF sessions");
        ptr = new T(args...);
   ...
   protected:
    ...
    void InitializeHistogramFromConfig(std::string name, ChannelPair channel,
    ...
    std::string concatenateNames(const std::string& baseName,
                                 const char* toAppend);
    std::string concatenateNames(const std::string& baseName,
                                 std::string& toAppend);
    ...
    std::string getHistName(std::string histName, std::string variationName,
                            std::string channel);
    template <typename T>
    void InitializeHistMap(std::vector<std::string>& labels,
                           std::unordered_map<HistLabel, T*>& histMap);

    // Filling Functions
    template <typename T, typename... Args>
    void SafeHistFill(std::unordered_map<HistLabel, T*>& container,
                      std::string name, Channel chan, Systematic var,
                      Args... args) {
    ...
    template <typename T, typename... Args>
    void SafeHistFill(std::unordered_map<HistLabel, T*>& container,
                      const char* name, Channel chan, Systematic var,
                      Args... args) {
    ...
    template <typename T, typename... Args>
    void HistFullFill(std::unordered_map<HistLabel, T*>& container,
                      const std::string& histname, Systematic var,
                      Args... args) {
